...
Pi and pi-signed crew launches capability-detect `--tui-mode` from `pi --help` and pass `--tui-mode regular` only on builds that still advertise it (Pi 0.83.0 removed the flag); the [harness-adapters skill](../.agents/skills/harness-adapters/SKILL.md) owns that behavior.
